...
# ── Stage7 매대 층 라우팅(--shelf-level): 상태머신은 SHELF3_* 상수만 참조하므로 값만 바꾸면
#    place 전체가 해당 층으로 라우팅됨. 실측(--inspect-shelf 2026-06-12):
#      3층=바닥1.14/앞턱1.15/천장없음(개방), 2층=바닥0.95/앞턱0.96/천장1.11(개구0.16m).
...
def main():
    ...
    if _obj_type == "cylinder" and args.objects > 1:
        ...
        for _i in range(args.objects):
            ...
            _op = f"/World/obj_{_i}"
            # 혼합 자세(--can-pose lying): 홀수 인덱스는 눕힘(축 y), 짝수는 직립 → 서있는+누운 동시 검증.
            _lying_i = (args.can_pose == "lying" and _i % 2 == 1)
            _ori_i = (np.array([0.70710678, 0.70710678, 0.0, 0.0]) if _lying_i
                      else np.array([1.0, 0.0, 0.0, 0.0]))
            _cz_i = (_table_top + s["radius"] + 0.002) if _lying_i else _cz
            _ob = _cyl.DynamicCylinder(
                prim_path=_op, name=f"obj_{_i}",
                position=np.array([_cx, _robot_base[1] + _dy_i, _cz_i]), orientation=_ori_i,
            ...
            print(f"[다물체] {_op} @ dy={_dy_i:+.2f} {'(눕힘)' if _lying_i else '(직립)'}", flush=True)
        ...
        if args.can_pose == "lying":
            _orient = np.array([0.70710678, 0.70710678, 0.0, 0.0])   # 90° about X(w,x,y,z)
